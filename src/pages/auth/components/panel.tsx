...
/** A titled section of the account or admin console. */
...
/** Loading, failure, no-access and empty rendered consistently across every list in the console. */
export const PanelState = ({loading, error, forbidden, empty, emptyLabel, onRetry, children}: PanelStateProps) => {
  ...
  if (forbidden) {
    return <Alert tone="info" title={t("auth:common.forbidden_title")}>{t("auth:common.forbidden_body")}</Alert>;
  }
    ...
    return (
      <Alert tone="error" title={t("auth:common.failed")}>
        <div className="flex flex-wrap items-center gap-3">
          <span>{t(`auth:errors.${error}`, {defaultValue: error})}</span>
          {onRetry && (
            <Button variant="ghost" size="sm" onClick={onRetry} data-fs-hover>
              <ArrowClockwise size={14}/> {t("auth:common.retry")}
            </Button>
          )}
        </div>
      </Alert>
  ...
  if (empty) {
    return <p className="py-4 text-sm text-neutral-500">{emptyLabel ?? t("auth:common.empty")}</p>;
  }
